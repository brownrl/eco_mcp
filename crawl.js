  ...
  for (const page of pagesToCrawl) {
    try {
      ...
      const pageId = await dbInsert(
        'INSERT INTO pages (url, title, html, category, hierarchy_1, hierarchy_2, hierarchy_3, hierarchy_4) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [page.url, title, html, page.category, h1, h2, h3, h4]
      );
      
      // Update FTS with lowercased content for case-insensitive search
      await dbRun(
        'INSERT INTO pages_fts(rowid, title, html) VALUES (?, LOWER(?), LOWER(?))',
        [pageId, title, html]
      );
